...
hardware_interface::CallbackReturn ODriveHardwareInterface::on_init(const hardware_interface::HardwareInfo & info)
{
  ...
  // Size the sensor related interfaces
  hw_vbus_voltages_.resize(info_.sensors.size(), std::numeric_limits<double>::quiet_NaN());
  hw_rt_interface.hw_vbus_voltages_.resize(info_.sensors.size(), std::numeric_limits<double>::quiet_NaN());
  // size axis related interfaces
  ...
  // for realtime interface
  hw_rt_interface.hw_positions_.resize(info_.joints.size(), std::numeric_limits<double>::quiet_NaN());
  hw_rt_interface.hw_velocities_.resize(info_.joints.size(), std::numeric_limits<double>::quiet_NaN());
  hw_rt_interface.hw_efforts_.resize(info_.joints.size(), std::numeric_limits<double>::quiet_NaN());
  hw_rt_interface.hw_commands_positions_.resize(info_.joints.size(), std::numeric_limits<double>::quiet_NaN());
  hw_rt_interface.hw_commands_velocities_.resize(info_.joints.size(), std::numeric_limits<double>::quiet_NaN());
  hw_rt_interface.hw_commands_efforts_.resize(info_.joints.size(), std::numeric_limits<double>::quiet_NaN());

  hw_rt_interface.hw_axis_errors_.resize(info_.joints.size(), std::numeric_limits<double>::quiet_NaN());
  hw_rt_interface.hw_motor_errors_.resize(info_.joints.size(), std::numeric_limits<double>::quiet_NaN());
  hw_rt_interface.hw_encoder_errors_.resize(info_.joints.size(), std::numeric_limits<double>::quiet_NaN());
  hw_rt_interface.hw_controller_errors_.resize(info_.joints.size(), std::numeric_limits<double>::quiet_NaN());
  hw_rt_interface.hw_fet_temperatures_.resize(info_.joints.size(), std::numeric_limits<double>::quiet_NaN());
  hw_rt_interface.hw_motor_temperatures_.resize(info_.joints.size(), std::numeric_limits<double>::quiet_NaN());
  hw_rt_interface.hw_motor_current_.resize(info_.joints.size(), std::numeric_limits<double>::quiet_NaN());
...
void ODriveHardwareInterface::usb_read_write_func() 
{
  ROS_INFO("USB Read Write Thread Started");
  // for calculating sleep time
  auto const period = std::chrono::nanoseconds(1'000'000'000 / 50);

  while (!_thread_terminate)
  {
    // calculate measured period
    auto const previous_time =  std::chrono::system_clock::now();

    // execute update loop
    read_into_rt_interface();  // read from hardware to rt interface structure
    write_from_rt_interface(); // write into hardware from rt interface
    auto const measured_period = std::chrono::system_clock::now() - previous_time;
    /// sleep only if required
    //if (measured_period < period) {
      std::this_thread::sleep_for(period);
    //}
  }
  ROS_INFO("USB Read write thread terminating");
}
...
hardware_interface::CallbackReturn ODriveHardwareInterface::on_activate(const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  usb_read_write_thread = std::thread([this] { this->usb_read_write_func() ; });
...
{
  // wait for read write thread to complete
  _thread_terminate = true;
  usb_read_write_thread.join();
...
hardware_interface::return_type ODriveHardwareInterface::read_into_rt_interface()
{
  if (rt_mutex_.try_lock()) {
    for (size_t i = 0; i < info_.sensors.size(); i++) {
      ...
      hw_rt_interface.hw_vbus_voltages_[i] = vbus_voltage;
    }
...
{
  // copy data from realtime interface
  rt_mutex_.lock();
  for (size_t i = 0; i < info_.sensors.size(); i++) {
    hw_vbus_voltages_[i] = hw_rt_interface.hw_vbus_voltages_[i];
  }

  for (size_t i = 0; i < info_.joints.size(); i++) {
    hw_efforts_[i]           = hw_rt_interface.hw_efforts_[i]           ;
    hw_velocities_[i]        = hw_rt_interface.hw_velocities_[i]        ; 
    hw_positions_[i]         = hw_rt_interface.hw_positions_[i]          ;
    hw_axis_errors_[i]       = hw_rt_interface.hw_axis_errors_[i]        ;
    hw_motor_errors_[i]      = hw_rt_interface.hw_motor_errors_[i]       ;
    hw_encoder_errors_[i]    = hw_rt_interface.hw_encoder_errors_[i]     ;
    hw_controller_errors_[i] = hw_rt_interface.hw_controller_errors_[i]  ;
    hw_fet_temperatures_[i]  = hw_rt_interface.hw_fet_temperatures_[i]   ;
    hw_motor_temperatures_[i]= hw_rt_interface.hw_motor_temperatures_[i] ;
    hw_motor_current_[i]     = hw_rt_interface.hw_motor_current_[i]      ;
  }
  rt_mutex_.unlock();
  return hardware_interface::return_type::OK;
}
...
hardware_interface::return_type ODriveHardwareInterface::write_from_rt_interface()
{
  if (hw_rt_interface.new_data_avail && rt_mutex_.try_lock()) {
    for (size_t i = 0; i < info_.joints.size(); i++) {
      std::string axis = "axis" + std::to_string(axes_[i]);
      uint64_t serial_num = serial_numbers_[1][i];
      odrive *od = odrives[serial_num];    
      ...
      switch (control_level_[i]) {
        case integration_level_t::POSITION:
          input_pos = (hw_rt_interface.hw_commands_positions_[i] / 2 / M_PI) * (reverse_control_[i] ? -1.0 : 1.0);
          ROS_DEBUG("::write %ld POSITION %f", i,input_pos);
          writeOdriveData(od->endpoint, od->json, axis + ".controller.input_pos",input_pos);
          break;
        case integration_level_t::VELOCITY:
          input_vel = (hw_rt_interface.hw_commands_velocities_[i] / 2 / M_PI) * (reverse_control_[i] ? -1.0 : 1.0);
          ROS_DEBUG("::write %ld VELOCITY %f", i,input_vel);
          writeOdriveData(od->endpoint, od->json, axis + ".controller.input_vel",input_vel);
          break;
        case integration_level_t::EFFORT:
          input_torque = hw_rt_interface.hw_commands_efforts_[i] * (reverse_control_[i] ? -1.0 : 1.0);
    ...
    hw_rt_interface.new_data_avail = false;
    rt_mutex_.unlock();
  }
    ...
    const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  for (size_t i = 0; i < info_.joints.size(); i++) {
    ...
    switch (control_level_[i]) {
      case integration_level_t::POSITION:
        if (hw_rt_interface.hw_commands_positions_[i] != hw_commands_positions_[i]) {
          ROS_DEBUG("updated position");
          hw_rt_interface.hw_commands_positions_[i] = hw_commands_positions_[i];
          hw_rt_interface.new_data_avail = true;
        }
        break;
      case integration_level_t::VELOCITY:
        if (hw_rt_interface.hw_commands_velocities_[i] != hw_commands_velocities_[i]) {
          ROS_DEBUG("updated velocity");
          hw_rt_interface.hw_commands_velocities_[i] = hw_commands_velocities_[i];
          hw_rt_interface.new_data_avail = true;
        }
        break;
      case integration_level_t::EFFORT:
        if (hw_rt_interface.hw_commands_efforts_[i] != hw_commands_efforts_[i]) {
          ROS_DEBUG("updated efforts");
          hw_rt_interface.hw_commands_efforts_[i] = hw_commands_efforts_[i];
          hw_rt_interface.new_data_avail = true;
        }
        break;
      case integration_level_t::UNDEFINED:
        ...
        break;
      default:
        ROS_INFO("control level UNKNOWN");
        break;
    }
  }
  rt_mutex_.unlock();
